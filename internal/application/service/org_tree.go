...
func (s *orgTreeService) CreateNode(ctx context.Context, tenantID uint64, userID string, req *types.CreateOrgTreeNodeRequest) (*types.Organization, error) {
	...
	if err := s.orgTreeRepo.Create(ctx, org); err != nil {
	...
	if org.ParentID == nil {
		...
		if err := s.orgTreeRepo.AddOrgTreeMember(ctx, creatorMember); err != nil {
...
func (s *orgTreeService) UpdateNode(ctx context.Context, nodeID string, tenantID uint64, req *types.UpdateOrgTreeNodeRequest) (*types.Organization, error) {
	...
	if err := s.orgTreeRepo.Update(ctx, org); err != nil {
...
func (s *orgTreeService) DeleteNode(ctx context.Context, nodeID string, tenantID uint64) error {
	...
	if err := s.orgTreeRepo.Delete(ctx, org.ID); err != nil {
...
func (s *orgTreeService) GetTree(ctx context.Context, tenantID uint64) ([]*types.OrgTreeNode, error) {
	...
	memberCounts, err := s.orgTreeRepo.BatchCountOrgTreeMembers(ctx, orgIDs)
	if err != nil {
		...
		for _, org := range orgs {
			members, err := s.orgTreeRepo.ListOrgTreeMembers(ctx, org.ID)
			if err != nil {
	...
	memberUserIDs, err := s.orgTreeRepo.BatchListOrgTreeMemberUserIDs(ctx, orgIDs)
	if err != nil {
	...
	for _, org := range orgs {
		node := &types.OrgTreeNode{
			...
			Level:             org.Level,
			SortOrder:         org.SortOrder,
			DirectMemberCount: int64(memberCounts[org.ID]),
			MemberCount:       int64(memberCounts[org.ID]), // will be updated to total after tree build
			CreatedAt:         org.CreatedAt,
			UpdatedAt:         org.UpdatedAt,
		}
		nodeMap[org.ID] = node
...
func (s *orgTreeService) computeSubtreeMemberCount(node *types.OrgTreeNode, memberUserIDs map[string][]string) map[string]bool {
	...
	node.TotalMemberCount = int64(len(userSet))
...
func (s *orgTreeService) AssignUserToOrg(ctx context.Context, orgID string, tenantID uint64, req *types.AssignUserToOrgRequest) error {
	...
	if err := s.orgTreeRepo.AddOrgTreeMember(ctx, member); err != nil {
		if err == repository.ErrOrgMemberAlreadyExists {
			// Update existing member's role
			if err := s.orgTreeRepo.UpdateOrgTreeMemberRole(ctx, orgID, req.UserID, req.Role); err != nil {
...
func (s *orgTreeService) RemoveUserFromOrg(ctx context.Context, orgID string, tenantID uint64, req *types.RemoveUserFromOrgRequest) error {
	...
	if s.tenantMemberService != nil {
		userOrgs, err := s.orgTreeRepo.ListOrgTreeOrganizationsByUserID(ctx, req.UserID)
		if err != nil {
	...
	if err := s.orgTreeRepo.RemoveOrgTreeMember(ctx, orgID, req.UserID); err != nil {
...
func (s *orgTreeService) SetOrgAdmin(ctx context.Context, orgID string, tenantID uint64, req *types.SetOrgAdminRequest) error {
	...
	if err := s.orgTreeRepo.UpdateOrgTreeMemberRole(ctx, orgID, req.UserID, role); err != nil {
		return fmt.Errorf("failed to update member role: %w", err)
	}
	return nil
}
...
func (s *orgTreeService) GetUserOrganizations(ctx context.Context, userID string, tenantID uint64) ([]*types.OrgTreeNode, error) {
	...
	allOrgs, err := s.orgTreeRepo.ListOrgTreeOrganizationsByUserID(ctx, userID)
	if err != nil {
	...
	for _, org := range allOrgs {
		if org.OrgTenantID != nil && *org.OrgTenantID == tenantID {
			...
			node := &types.OrgTreeNode{
				...
				Level:             org.Level,
				SortOrder:         org.SortOrder,
				DirectMemberCount: int64(len(members)),
				TotalMemberCount:  int64(len(members)),
...
func (s *orgTreeService) ListOrgMembers(ctx context.Context, orgID string, tenantID uint64) ([]*types.OrganizationMember, error) {
	...
	members, err := s.orgTreeRepo.ListOrgTreeMembers(ctx, orgID)
	if err != nil {
...
func (s *orgTreeService) IsAdminOfAnyOrg(ctx context.Context, userID string, orgIDs []string, tenantID uint64) bool {
	return s.orgTreeRepo.IsAdminOfAnyOrgTree(ctx, userID, orgIDs, tenantID)
}
...
func (s *orgTreeService) ListInheritedAdmins(ctx context.Context, orgID string, tenantID uint64) ([]map[string]interface{}, error) {
	...
	for _, ancestorID := range ancestorIDs {
		members, err := s.orgTreeRepo.ListOrgTreeMembers(ctx, ancestorID)
		if err != nil {
			continue
		}
	...
	directMembers, _ := s.orgTreeRepo.ListOrgTreeMembers(ctx, orgID)
