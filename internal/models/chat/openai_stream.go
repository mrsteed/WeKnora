...
func (c *RemoteAPIChat) processToolCallsDelta(
	...
	for _, tc := range toolCalls {
		...
		// Stream final_answer tool arguments as answer-type chunks so the
		// session stream can surface authoritative answer text incrementally
		// even when the provider emits it inside tool-call arguments.
