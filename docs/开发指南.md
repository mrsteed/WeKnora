...
未设置 `WEKNORA_DEV_DATA_ROOT` 时，开发环境容器会沿用历史 Docker named volumes。若要恢复宿主机目录挂载，推荐使用 `make dev-start WEKNORA_DEV_DATA_ROOT=/data/weknora`；也可替换为自定义绝对路径。详细映射关系、目录结构与权限说明参见 [开发环境数据目录挂载说明](开发环境数据目录挂载说明.md)。
...
脚本方式也支持宿主机目录挂载：`WEKNORA_DEV_DATA_ROOT=/data/weknora ./scripts/dev.sh start`。如需自定义，可替换为其他绝对路径。
