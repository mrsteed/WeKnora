...
未设置 `WEKNORA_DEV_DATA_ROOT` 时，`make dev-start` 会沿用历史 Docker named volumes。如需切到宿主机目录模式，推荐使用 `make dev-start WEKNORA_DEV_DATA_ROOT=/data/weknora`；也可替换为自定义绝对路径。详细说明参见 [开发环境数据目录挂载](开发环境数据目录挂载.md)。
...
- [开发环境数据目录挂载](开发环境数据目录挂载.md) — 开发模式的宿主机挂载约定
...
- [开发环境数据目录挂载](开发环境数据目录挂载.md) — 开发模式容器数据目录说明
