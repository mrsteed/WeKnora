...
LOG_DIR="${SPEACHES_ASR_CPU_SCRIPT_LOG_DIR:-${SCRIPT_DIR}/.logs}"
mkdir -p "${LOG_DIR}"
...
compress_cpuset() {
  ...
  local joined=''
  local part
  for part in "${ranges[@]}"; do joined+="${joined:+,}${part}"; done
  printf '%s' "${joined}"
}
...
IMAGE="${SPEACHES_ASR_CPU_IMAGE:-ghcr.io/speaches-ai/speaches:latest-cpu}"
INSTANCE_COUNT="${SPEACHES_ASR_CPU_INSTANCES:-2}"
CONTAINER_PORT="${SPEACHES_ASR_CPU_CONTAINER_PORT:-8000}"
BASE_PORT="${SPEACHES_ASR_CPU_BASE_PORT:-18000}"
CACHE_ROOT="${SPEACHES_ASR_CPU_CACHE_ROOT:-/data/models/hf-cache/speaches-asr-cpu}"
RECOMMENDED_MODEL="${SPEACHES_ASR_CPU_RECOMMENDED_MODEL:-Systran/faster-whisper-medium}"
STOP_TIMEOUT="${SPEACHES_ASR_CPU_STOP_TIMEOUT:-45}"
DOCKER_RESTART_POLICY="${SPEACHES_ASR_CPU_RESTART_POLICY:-unless-stopped}"
HEALTH_PATH="${SPEACHES_ASR_CPU_HEALTH_PATH:-/health}"
dry_run=0
...
load_instance_config() {
  INST_NAME=(); INST_PORT=(); INST_CPUSET_CPUS=(); INST_CPUSET_MEMS=(); INST_THREADS=()
  ...
  for (( idx=0; idx<INSTANCE_COUNT; idx++ )); do
    ...
    local name_var="SPEACHES_ASR_CPU_INSTANCE_${idx}_NAME"
    local port_var="SPEACHES_ASR_CPU_INSTANCE_${idx}_PORT"
    local cpus_var="SPEACHES_ASR_CPU_INSTANCE_${idx}_CPUSET_CPUS"
    local mems_var="SPEACHES_ASR_CPU_INSTANCE_${idx}_CPUSET_MEMS"
    local threads_var="SPEACHES_ASR_CPU_INSTANCE_${idx}_THREADS"
    ...
    INST_NAME[idx]="${!name_var:-weknora-speaches-asr-cpu-${suffix}}"
...
target_matches() {
  ...
  [[ "${target}" == "all" || "${target}" == "${idx}" || "${target}" == "${INST_NAME[$idx]}" ]]
}
...
preflight() {
  ...
  load_instance_config
  local idx
  log_info "Speaches CPU 镜像: ${IMAGE}"
  log_info "推荐请求 model: ${RECOMMENDED_MODEL}"
  for (( idx=0; idx<INSTANCE_COUNT; idx++ )); do
    log_info "实例 ${idx}: name=${INST_NAME[$idx]} port=${INST_PORT[$idx]} cpus=${INST_CPUSET_CPUS[$idx]} mems=${INST_CPUSET_MEMS[$idx]} threads=${INST_THREADS[$idx]}"
  done
}
...
run_container_for_instance() {
  ...
  local -a cmd=(
    ...
    --label weknora.model.cpu=speaches-asr
    ...
    -e UVICORN_PORT="${CONTAINER_PORT}"
    -e OMP_NUM_THREADS="${threads}"
    -e MKL_NUM_THREADS="${threads}"
    -e OPENBLAS_NUM_THREADS="${threads}"
    -e TOKENIZERS_PARALLELISM=false
    -v "${cache_dir}:/home/ubuntu/.cache/huggingface/hub"
    "${IMAGE}"
  )
...
start_instance() {
  ...
  log_inst "${idx}" "Speaches 按请求动态加载模型；推荐请求 model=${RECOMMENDED_MODEL}"
}
...
logs_for() {
  ...
  for (( idx=0; idx<INSTANCE_COUNT; idx++ )); do
    ...
    if container_exists_for "${INST_NAME[$idx]}"; then
      "${docker_cmd[@]}" logs --tail "${SPEACHES_ASR_CPU_LOG_TAIL:-200}" -f "${INST_NAME[$idx]}" 2>&1 | sed -u "s/^/[${INST_NAME[$idx]}] /" &
    else
...
默认实例:
  0 -> weknora-speaches-asr-cpu-a  port=18000
  1 -> weknora-speaches-asr-cpu-b  port=18001

关键环境变量:
  SPEACHES_ASR_CPU_IMAGE                 默认 ghcr.io/speaches-ai/speaches:latest-cpu
  SPEACHES_ASR_CPU_BASE_PORT             默认 18000
  SPEACHES_ASR_CPU_RECOMMENDED_MODEL     默认 Systran/faster-whisper-medium
  SPEACHES_ASR_CPU_CACHE_ROOT            默认 /data/models/hf-cache/speaches-asr-cpu
  SPEACHES_ASR_CPU_INSTANCE_<idx>_PORT   单实例宿主机端口覆盖
  SPEACHES_ASR_CPU_INSTANCE_<idx>_CPUSET_CPUS / _CPUSET_MEMS / _THREADS
...
示例:
  ...
  ./${SCRIPT_NAME} reset weknora-speaches-asr-cpu-b
USAGE_EOF
}
