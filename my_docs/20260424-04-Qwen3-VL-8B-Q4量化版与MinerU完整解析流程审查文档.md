# Qwen3-VL-8B Q4 量化版与 MinerU 并存时的“问文档”完整解析流程审查文档

## 1. 审查范围与前提

本文基于当前仓库代码，审查以下场景下“问文档”知识库导入的真实执行流程：

1. 已部署 Qwen3-VL-8B Q4 量化版。
...
1. 当前系统中，MinerU 和 Qwen3-VL-8B Q4 量化版不在同一层工作。
2. MinerU 属于“文档解析引擎层”，负责把文件转成 Markdown 和图片引用。
3. Qwen3-VL-8B Q4 量化版在你当前假设下属于“知识库 VLLM 多模态后处理层”，负责对已经提取出来的图片做 OCR 与 Caption。
4. 即使 MinerU 服务已经启用，也只有当知识库 `ParserEngineRules` 把对应文件类型指向 `mineru` 时，导入流程才会真正走 MinerU。
5. 只配置了 WeKnora 的 VLLM 模型，并不会把 Qwen3-VL-8B Q4 量化版直接注入到 MinerU 解析引擎内部。
6. 在某些文档类型上，MinerU 已经完成过 OCR，而后续 Qwen3-VL-8B Q4 量化版又会对抽取出的图片再做一次 OCR，因此当前架构仍然存在“二次 OCR”可能。
7. “Q4 量化版”改变的是模型部署形态、显存占用、吞吐与边缘精度，不改变 WeKnora 当前的链路结构与调用位置。
...
### 3.2 Qwen3-VL-8B Q4 量化版在当前假设下负责什么

当你把 Qwen3-VL-8B Q4 量化版配成知识库的 `VLLM` 模型后，它的主要执行点在：
...
这意味着当前 Qwen3-VL-8B Q4 量化版并不替代 MinerU，而是运行在 MinerU 之后。

与 DeepSeek-OCR-2 相比，Qwen3-VL-8B 更偏通用视觉理解模型，因此它在“图片描述、图文理解、开放式视觉问答”上的适配性通常更自然；但由于这里使用的是 Q4 量化版，小字、复杂表格、密集公式等场景的边缘精度仍需要单独验证。
...
9. 每个任务使用知识库配置的 VLLM 模型，也就是你配置的 Qwen3-VL-8B Q4 量化版，对图片做 OCR 与 Caption。
10. 所有图片处理完成后，系统再触发 `knowledge:post_process`，把知识状态改成 `completed`，并继续派发摘要、问题生成、图谱抽取等任务。

这是当前仓库中“MinerU + Qwen3-VL-8B Q4 量化版 VLLM”并存时的真实闭环。
...
WeKnora 当前不会把知识库 `VLMConfig` 中配置的 Qwen3-VL-8B Q4 量化版地址、模型名、API Key 传给 MinerU。
...
因此你如果“只是在 WeKnora 模型管理里把 Qwen3-VL-8B Q4 量化版配成了 VLLM 模型”，MinerU 本身并不会自动改用它。
...
因此，在当前代码里，Qwen3-VL-8B Q4 量化版不会只做 OCR；它会同时被用于 OCR 和 Caption。

### 5.9 第九步：Qwen3-VL-8B Q4 量化版真正开始工作

关键实现：

- [internal/application/service/image_multimodal.go](../internal/application/service/image_multimodal.go)

这是你配置的 Qwen3-VL-8B Q4 量化版真正被调用的地方。

流程如下：

1. `resolveVLM()` 读取知识库 `VLMConfig`。
2. 如果配置的是新式 `ModelID`，通过 `modelService.GetVLMModel()` 获取模型实例。
3. 如果该 `ModelID` 对应的就是 Qwen3-VL-8B Q4 量化版，那么这里得到的就是该模型。
4. 对同一张图片执行两次推理：
   - 用 OCR Prompt 提取正文
   - 用 Caption Prompt 生成中文描述
5. 结果分别写入新的 chunk 并再次建索引。

因此，Qwen3-VL-8B Q4 量化版在当前系统中的真实位置是：

“文档已经被解析并提取出图片之后的知识增强阶段”。

需要额外指出的是：Q4 量化版只影响模型服务内部的推理表现，不影响 WeKnora 这里的调用协议。对 WeKnora 而言，它仍然只是一个被 `resolveVLM()` 选中的视觉模型实例。
...
9. 异步任务使用知识库的 Qwen3-VL-8B Q4 量化版做第二阶段 OCR 与 Caption。
...
3. 如果返回了图片页或图内图片，后续多模态任务仍会再对这些图片跑 Qwen3-VL-8B Q4 量化版。

这类文件最容易出现“MinerU OCR + Qwen3-VL-8B Q4 量化版 OCR 双阶段并存”。
...
1. MinerU 先完成文档结构化。
2. 文档内图片被抽取并落存储。
3. Qwen3-VL-8B Q4 量化版再对这些图片做 OCR 与 Caption。
...
2. 系统仍会保留原图引用。
3. 后续多模态任务再用 Qwen3-VL-8B Q4 量化版做 OCR 与 Caption。

如果图片类型没有命中 `mineru`，则可能走 `simple`，此时第一阶段只保留图片引用，真正的 OCR 基本全部落到 Qwen3-VL-8B Q4 量化版异步任务里。
...
3. 若 Markdown 内含图片链接并被成功解析入库，仍可触发 Qwen3-VL-8B Q4 量化版多模态任务。
...
### 8.2 发现二：Qwen3-VL-8B Q4 量化版当前不会直接驱动 MinerU

你在 WeKnora 模型中心里配置的 Qwen3-VL-8B Q4 量化版，只会进入知识库 `VLMConfig` 链路。

当前代码没有把这份 VLM 配置传给 MinerU 解析器。

所以当前系统更准确的描述不是：

“MinerU 使用了 Qwen3-VL-8B Q4 量化版解析文档”

而是：

“MinerU 负责首轮解析，Qwen3-VL-8B Q4 量化版负责解析后的图片增强”。
...
5. Qwen3-VL-8B Q4 量化版已配置为知识库 VLM 模型

系统就可能出现：

1. MinerU 在文档解析阶段先做了一轮 OCR。
2. Qwen3-VL-8B Q4 量化版在图片多模态阶段又做一轮 OCR。
...
### 8.4 发现四：Q4 量化版更适合被视为部署参数，而不是流程分支

当前代码既不会识别“这个模型是不是 Q4”，也不会因为它是量化版而走不同分支。
...
所以这次模型替换的本质是“能力侧权衡变化”，不是“链路结构变化”。

## 9. 对当前部署方式的判断

基于当前代码，如果你的部署方式是：

1. WeKnora 中配置了 Qwen3-VL-8B Q4 量化版为 VLLM 模型。
2. 知识库启用了多模态。
...
“MinerU 负责文档第一阶段结构化解析，Qwen3-VL-8B Q4 量化版负责文档图片第二阶段语义 OCR 与说明增强。”

这仍然是一种串联架构，而不是单层替代架构。

相较于 DeepSeek-OCR-2 场景，这里有一个额外好处：

Qwen3-VL-8B 本身更偏通用视觉模型，所以它被同一条链路同时用于 OCR 与 Caption，在职责匹配上通常更顺滑；但由于是 Q4 量化版，仍需要重点观察高密度文档场景下的识别保真度。
...
1. MinerU 是首轮解析器。
2. Qwen3-VL-8B Q4 量化版是后处理增强器。
3. 对扫描 PDF、图文文档、复杂表格重点做效果验证。
...
2. 或者对 MinerU 已明确完成 OCR 的结果，跳过 Qwen3-VL-8B Q4 量化版 OCR 子任务。

### 10.3 如果你担心 Q4 量化影响精度
...
### 10.4 如果你希望 Qwen3-VL-8B Q4 量化版成为真正的“主 OCR 引擎”

则需要进一步改造，把当前 KB 级 VLM 配置真正下沉到 parser engine 层，或者把 MinerU 和 VLM 之间的责任边界重新拆分。

在当前代码里，这件事还没有完成。

## 11. 最终结论

在“已部署 Qwen3-VL-8B Q4 量化版 + 已配置 VLLM 模型 + 已启用 MinerU 服务”的前提下，当前系统的完整解析流程是：

1. 先根据知识库规则决定是否使用 MinerU。
2. 若命中 MinerU，则由 MinerU 执行第一阶段文档解析。
3. 解析结果中的图片被保存并进入异步多模态任务。
4. 异步任务再调用知识库配置的 Qwen3-VL-8B Q4 量化版做 OCR 与 Caption。
5. 最终由统一后处理任务将知识状态改为完成，并继续摘要、问题生成、图谱等后续任务。

因此，当前系统中 Qwen3-VL-8B Q4 量化版与 MinerU 的关系不是二选一，而是“前后串联、分层协作”。

并且与 DeepSeek-OCR-2 版本相比，这次替换带来的核心变化主要在模型能力取向与量化精度权衡上，而不在当前 WeKnora 的流程控制结构上。