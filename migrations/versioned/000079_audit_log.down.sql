-- Migration 000079 down: drop audit_logs table.
--
...
BEGIN
    IF coalesce(current_setting('weknora.allow_destructive_migration', true), 'false') <> 'true' THEN
        RAISE EXCEPTION
            '[Migration 000079 down] BLOCKED: refuse to drop audit_logs. '
            'Set weknora.allow_destructive_migration = ''true'' in this session to override.';
    END IF;
    RAISE WARNING '[Migration 000079 down] Dropping audit_logs — durable history will be lost';
END $$;
...
DO $$ BEGIN RAISE NOTICE '[Migration 000079 down] audit_logs dropped'; END $$;
