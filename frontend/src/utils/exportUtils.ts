import { postDown, get } from './request';
...
const exportViaBackend = async (
  ...
  let blob: Blob;
  try {
    blob = await postDown(`${buildExportApiBase(exportApiBase)}/document`, {
      content,
      format,
  ...
  const downloadFilename = buildFallbackFilename(filename, format);
