export default {
  ...
  integrations: {
    ...
    claw: {
      ...
      subtitle: '通过 IntraMind REST API 导入文档并执行混合检索（向量 + 关键词）。适用于上传文件/URL/Markdown 到知识库、跨库检索与浏览知识内容。',
      ...
      installCtaHint: '安装 IntraMind Skill · 将在新标签页打开',
    ...
    chrome: {
      title: '知识管理助手',
      subtitle: '配合 IntraMind 自建服务使用：在任意网页侧边栏提问、剪藏内容、Markdown 速记，将浏览中的知识沉淀到你的知识库。',
      ...
      steps: {
        ...
        port: {
          title: '桌面版配置固定端口（推荐）',
          desc: '使用 IntraMind 桌面版时，在 API 信息中设置固定端口（如 37841），避免每次启动后地址变化导致插件断连。'
        },
        api: {
          title: '获取 API 凭证',
          desc: '在「设置 → API 信息」中复制 API Key 与 API 地址。'
    ...
    api: {
      ...
      signedFlowDetail: 'HMAC 密钥仅保存在 IntraMind 与你的可信后端，切勿放入请求或下发给浏览器。请求头里传的是用该密钥签出的 JWT 字符串（每次签发或过期后会变），不是密钥本身。JWT 须包含 sub（外部用户 ID）、tenant_id、aud=weknora、exp（有效期不超过 24 小时）。',
      ...
      hmacSecret: 'HMAC 密钥',
      hmacSecretDesc: '与 IntraMind 保存的同一份密钥；仅用于后端签发 JWT，不要作为请求头发送。',
  ...
  platform: {
    subtitle: '把散落文档，变成活的知识大脑',
    description: '能回答、能推理、能自生长——你的企业内脑',
    rag: 'RAG 增强生成',
    agent: 'ReAct 智能体',
    wiki: 'Wiki 知识库',
    hybridSearch: '混合检索',
    ...
    tagAnswer: '能回答 · 答案带引用',
    tagReason: '能推理 · 多步拆解',
    tagGrow: '能自生长 · 自动 Wiki',
    tagPrivate: '全栈私有化',
    loginFeatAnswer: '多模态解析：PDF / 扫描件 / 表格 / 音视频',
    loginFeatReason: '混合检索：向量 + BM25 + 知识图谱',
    loginFeatGrow: '自动 Wiki：知识自己生长',
    trustLine: '私有化部署 · 20+ 模型厂商 · 审计日志',
  ...
  chat: {
    ...
    referenceDownloadDenied: '当前账号没有该文档的下载权限，请联系知识库管理员',
    chunkIdLabel: '片段ID:',
  ...
  embedPublish: {
    ...
    webhookUrlDesc: '可选。访客发消息与助手回复完成时，IntraMind 会向该 HTTPS 地址 POST 事件（message_sent / message_received）。',
  ...
  auth: {
    ...
    firstTime: '首次使用 IntraMind？',
    registerSuccess: '注册成功，请登录',
    registerFailed: '注册失败',
    subtitle: '登录，继续你的探索',
    // 登录卡副标题下方补充说明（使用 platform.description）
    registerSubtitle: '创建账户并开始使用 IntraMind',
  ...
  settings: {
    ...
    sandbox: {
      ...
      backendDescriptions: {
        ...
        docker: '每次在当前 IntraMind 主机启动一个短生命周期容器执行脚本',
        local: '直接在 IntraMind 服务进程所在环境执行脚本',
      },
      addConfig: '添加沙箱后端',
      ...
      weknoraTemplateTitle: 'IntraMind 标准模板',
      weknoraDockerImage: 'IntraMind 标准镜像',
      weknoraDockerImageHint: '每次执行都会基于该镜像创建一个隔离的短生命周期容器，不保留会话沙箱。',
      localRuntimeWarning: '本地进程会直接在 IntraMind 服务器上执行技能脚本，不具备容器或 MicroVM 隔离。仅建议在可信的开发空间使用。',
      weknoraTemplateOverview: 'IntraMind 负责提供标准运行环境。连接集群后会自动发现模板，缺少标准模板时自动创建。',
      weknoraTemplateDescription: '包含 IntraMind 技能所需的 Python、Node.js、CLI 工具、工作目录和非 root 执行用户。',
      recommendedTag: '推荐',
  ...
  knowledgeBase: {
    ...
    folderTree: {
      ...
      createFailed: '目录创建失败',
      delete: '删除目录',
      deleteConfirm: '确定要删除空目录“{name}”吗？',
      deleteBlocked: '目录 {name} 内还有 {count} 篇文档，不能直接删除，请先移动或删除文档',
      deleteSuccess: '目录已删除',
      deleteFailed: '目录删除失败',
    },
    moveToFolder: {
    ...
    batchDownload: '批量下载',
    batchDownloadDownloading: '正在打包下载…',
    batchDownloadSuccess: '已打包下载 {count} 个文件',
  ...
  newUserGuide: {
    ...
    steps: {
      ...
      knowledge: {
        title: '创建你的知识库',
        desc: '知识库是一切的起点：上传文档、网页或 FAQ，IntraMind 会自动解析并建立索引。点击这里进入知识库。'
      },
      welcome: {
  ...
  admin: {
    ...
    org: {
      ...
      parentOrgLabel: "上级组织",
      parentOrgPlaceholder: "请选择上级组织（置空则为根组织）",
      parentInvalid: "不能选择自身或其下级组织作为上级",
      sortOrder: "排序",
