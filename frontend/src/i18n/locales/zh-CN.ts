export default {
  ...
  contextualGuide: {
    ...
    agentCreate: {
      steps: {
        ...
        navKnowledge: {
          title: "关联知识库",
          desc: "决定智能体可检索哪些知识。默认「全部可访问知识库」，也可改为指定库或暂不关联。",
        },
        knowledge: {
          title: "知识库范围",
  ...
  knowledgeBase: {
    ...
    downloadSourceUnsupported: "当前内容不支持下载源文件",
    downloadFile: "下载文件",
    deleteFile: "删除文件",
  ...
  agent: {
    ...
    pageShare: {
      manageTitle: "智能体共享",
      manageDesc: "配置智能体的共享方式，支持链接分享给外部访客，也可共享到空间供成员协作使用。",
      sectionTitle: "链接分享",
      sectionDesc: "开启后可生成专属访问链接，访客通过链接以只读方式使用该智能体。",
      emptyTitle: "尚未开启链接分享",
      emptyDesc: "开启后可生成专属访问链接，邀请他人只读使用该智能体。",
      enable: "启用链接分享",
      disable: "停用链接分享",
      enableSuccess: "链接分享已启用",
      disableSuccess: "链接分享已停用",
      linkLabel: "访问链接",
      openLink: "打开链接",
      shareCode: "分享码",
      createdAt: "创建时间",
      ...
      disabledHint: "链接分享已停用，访客将无法通过链接访问该智能体。",
      statusActive: "已启用",
      statusDisabled: "已停用",
      statusExpired: "已过期",
      notReady: "智能体尚未完成链接分享所需的配置，请稍后再试。",
    ...
    editor: {
      ...
      editTitle: "编辑智能体",
      basicInfo: "基本信息",
      basicInfoDesc: "配置智能体的名称、描述、可见性与运行模式",
      promptsConfig: "提示词",
      ...
      visibilityTip: "组织可见：自己所属的组织及其子组织可见。全局可见：当前空间内所有组织可见。",
      organizationLabel: "归属组织",
      orgRequired: "当前账号未加入任何组织，无法设置为组织可见",
      baseType: "基础类型",
      ...
      allKnowledgeBases: "全部可访问知识库",
      allKnowledgeBasesDesc: "智能体可访问当前账号已授权的全部知识库",
  ...
  tenant: {
    ...
    setupWizard: {
      title: "空间初始化向导",
      subtitle: "继续完成「{name}」的首位负责人和组织树初始化。",
      tip: "先开通 tenant member，再分配组织树角色，避免新用户登录后没有空间权限。",
      skip: "跳过并进入空间",
      enter: "进入空间",
      finish: "执行初始化",
      introDesc: "选择已有账号、新建用户或生成邀请链接，并为其设置空间级角色。",
      modeLabel: "负责人开通方式",
      tenantRoleLabel: "空间级角色",
      tenantRoleDesc: "该角色决定对方登录当前空间后的管理范围。",
      existingEmailLabel: "已有账号邮箱",
      existingEmailPlaceholder: "例如：name{'@'}example.com",
      existingEmailHint: "用户必须已在系统中注册；如果尚未注册，请改用“新建用户”或“邀请链接”。",
      inviteMessageLabel: "邀请说明（可选）",
      inviteMessagePlaceholder: "例如：请先完成注册，再联系管理员分配部门",
      rootOrgTitle: "组织树初始化",
      rootOrgDesc: "为新空间创建首个根组织，并按需把负责人挂进去。",
      createRootToggle: "同时创建根组织",
      rootOrgRequiredHint: "新建用户模式依赖组织节点；系统会先创建根组织，再完成账号开通。",
      rootOrgNameLabel: "根组织名称",
      rootOrgNamePlaceholder: "例如：{name}",
      rootOrgDescLabel: "根组织说明（可选）",
      rootOrgDescPlaceholder: "例如：默认根部门 / 总部",
      orgRoleLabel: "组织角色",
      orgRoleDesc: "组织角色只影响组织树内的人员与资源作用域，不等同于空间管理员角色。",
      inviteOrgHint: "邀请链接模式暂不自动分配组织角色。对方注册加入后，再到组织树页面挂载到对应节点。",
      retryReuseRootHint: "根组织已创建，本次重试会复用「{name}」。",
      resultTitle: "初始化已完成",
      resultDesc: "空间已切换到「{name}」，现在可以直接进入并继续配置。",
      resultOrgCreated: "已创建根组织「{name}」。",
      resultOrgSkipped: "本次未创建组织树节点。",
      resultExisting: "已把 {email} 加入当前空间。",
      resultExistingWithOrg: "已把 {email} 加入当前空间，并挂到「{org}」。",
      resultNewUserWithOrg: "已创建账号「{username}」，并分配到「{org}」。",
      resultInvite: "已生成可复制的邀请链接，接收方完成注册后即可加入当前空间。",
      ...
      modes: {
        existing: {
          title: "已有账号",
          desc: "按邮箱把已注册用户加入当前空间",
        },
        new: {
          title: "新建用户",
          desc: "直接创建账号、开通空间权限并挂到组织节点",
        },
        invite: {
          title: "邀请链接",
          desc: "生成注册链接，稍后由对方自行完成注册加入",
        },
      },
      errors: {
        rootOrgNameRequired: "请输入根组织名称",
        existingEmailRequired: "请输入已有账号邮箱",
        existingUserNotFound: "未找到该系统账号，请改用“新建用户”或“邀请链接”",
        generic: "初始化失败，请稍后重试",
        missingInviteLink: "邀请链接生成成功，但未返回可复制地址",
      },
      warnings: {
        newUserPartial: "账号已创建并加入空间，但组织分配未完全成功：{message}",
      },
    },
    details: {
      idLabel: "空间 ID",
  ...
  knowledgeEditor: {
    ...
    database: {
      title: "数据库连接",
      description: "配置数据库知识库的连接信息，并限制可访问的表和字段范围。",
      engineLabel: "数据库引擎",
      engineTip: "选择要接入的数据库类型。当前支持 MySQL 和 PostgreSQL。",
      dataSourceNameLabel: "数据源名称",
      dataSourceNamePlaceholder: "请输入数据源名称（可选）",
      dataSourceNameTip: "仅用于知识库内部标识；留空时默认使用知识库名称。",
      hostLabel: "主机地址",
      ...
      schemaLabel: "Schema",
      schemaPlaceholder: "请输入 Schema 名称",
      usernameLabel: "用户名",
      usernamePlaceholder: "请输入数据库用户名",
      passwordLabel: "密码",
      passwordPlaceholder: "请输入数据库密码",
      sslModeLabel: "SSL 模式",
      sslModePlaceholder: "例如：false、require、disable",
      sslModeTip: "MySQL 常用 false / true / skip-verify；PostgreSQL 常用 disable / require / verify-ca。",
      queryConstraintTitle: "查询约束",
      queryConstraintDescription: "限制可访问的数据范围，降低误查风险。每行填写一个表名或字段名；留空表示不额外限制。",
      tableAllowlistLabel: "允许访问的表",
      tableAllowlistPlaceholder: "每行一个表名，例如：orders",
      tableAllowlistTip: "仅允许访问这些表。留空时表示允许访问当前库中的所有表。",
      columnDenylistLabel: "禁止访问的字段",
      columnDenylistPlaceholder: "每行一个字段名，例如：users.password",
      columnDenylistTip: "这些字段会在 schema 和查询阶段被过滤。建议填写密码、密钥、身份证号等敏感字段。",
      maxRowsLabel: "单次最大返回行数",
      maxRowsPlaceholder: "例如：200",
      timeoutLabel: "查询超时（秒）",
      timeoutPlaceholder: "例如：15",
      testConnection: "测试连接",
      testSuccess: "连接测试通过",
      testFailed: "连接测试失败",
  ...
  admin: {
    ...
    member: {
      ...
      roleEditor: "协作者",
      roleAdmin: "管理员",
      roleSubOrgAdmin: "子组织管理员",
      adjustInTenantMembers: "请在空间成员页面调整该成员的空间级角色",
    ...
    org: {
      ...
      descriptionLabel: "组织描述",
      descriptionPlaceholder: "请输入组织描述（选填）",
      sortOrder: "排序",
      ...
      createSuccess: "创建成功",
      updateSuccess: "更新成功",
      deleteConfirm: "确定删除该组织吗？删除后无法恢复。",
      deleteSuccess: "删除成功",
      deleteFailed: "删除失败",
      moveSuccess: "移动成功",
      moveFailed: "移动失败",
  ...
  agentEditor: {
    ...
    tools: {
      ...
      externalDbSchema: "查看外部数据库结构",
      externalDbSchemaDesc: "获取数据库知识库中的表结构、字段与查询约束",
      externalDbSearchTables: "检索外部数据库表",
      externalDbSearchTablesDesc: "按表名、表注释、列名和列注释检索候选表",
      externalDbQuery: "查询外部数据库",
      externalDbQueryDesc: "对数据库知识库执行只读 SQL 查询并返回结果",
      dataAnalysis: "数据分析",
      ...
      requiresDatabaseKb: "（需要关联数据库类型知识库）",
  ...
  integrations: {
    ...
    api: {
      title: "API 集成",
      subtitle: "通过 REST API 接入，并配置请求如何标识最终用户。",
      ...
      baseUrlDesc: "将此基础地址与 REST API 路径组合使用。",
      apiKey: "空间 API Key",
      apiKeyDesc: "API Key 仍然是空间级别；最终用户身份由下方模式提供。",
      principalMode: "用户身份模式",
      principalModeDesc:
        "配置 API 请求如何标识最终用户。该身份会按用户隔离对话 Session 和 MCP 工具授权，而不会削弱 API Key 的空间管理员访问能力。",
      principalScope:
        "最终用户身份会按用户隔离对话 Session 和 MCP 工具授权（OAuth）。API Key 仍可访问空间内所有知识库；API 权限不会按外部用户拆分。",
      modeTenant: "仅空间",
      modeDirect: "直接用户 ID",
      modeSigned: "签名 Token",
      directWarning: "直接用户 ID 模式信任调用方提供的 Header。仅用于受信任的服务端到服务端调用。",
      directWarningDetail:
        "任何持有 API Key 的调用方都可以修改用户 ID Header，冒充其他外部用户并复用或劫持该用户的 MCP OAuth 授权。不要在浏览器或不受信客户端中使用；面向最终用户的应用请使用签名 Token。",
      signedRecommended: "推荐用于面向用户的应用：由你的业务后端为外部用户签发短期 HS256 JWT。",
      signedFlowDetail:
        "HMAC secret 只保存在 WeKnora 和可信后端，不能放进请求，也不要发到浏览器。请求头里传的是用该 secret 签名后的 JWT 字符串（每次签发或过期后都会变化），而不是 secret 本身。JWT 必须包含 sub（外部用户 ID）、tenant_id、aud=weknora 和 exp（最长 24 小时）。",
      directHeader: "用户 ID Header",
      requireDirectHeader: "要求用户 ID Header",
      requireDirectHeaderDesc:
        "开启后，没有用户 ID Header 的 API Key 请求会被拒绝；关闭后，所有请求都按整个空间处理，不区分最终用户。",
      tokenHeader: "Token Header",
      tokenHeaderDesc: "客户端传入由后端签名的 JWT，Header 名称可配置。",
      hmacSecret: "HMAC secret",
      hmacSecretDesc: "与 WeKnora 中保存的 secret 相同；仅在你的后端用于签发 JWT，绝不要作为请求 Header 发送。",
      secretConfigured: "Secret 已配置；输入新值可轮换",
      generateSecret: "生成 secret",
      tokenSignExample: "后端 JWT 签名示例（Go）",
      signedRequestStep0:
        "# 0. 在你的后端签发 JWT（HS256，aud=weknora，sub=用户 ID，tenant_id={tenantId}，exp<=24h）",
      signedRequestStep0Hint: "#    把 JWT 放进 {headerName}，不要发送 HMAC secret",
      requestExampleCreateSession: "# 1. 创建会话",
      requestExampleAgentChat: "# 2. Agent 对话（SSE；把 <session_id> 替换为第 1 步得到的 id）",
      requestExampleJwtPlaceholder: "<由你的后端签名的 JWT>",
      requestExample: "请求示例",
